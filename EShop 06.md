...
Удалим класс `JsonProductRepository` в `EShop.DAL`  и `Products.json` в` EShop.Application`  они нам больше не понадобятся - наше приложение теперь использует БД для хранения данных  
## ProductsController 
...
Изменим возвращаемые значения.    
![](attachments/Pasted%20image%2020250207153932.png)
Что нам дало такое изменение?  
Теперь мы можем в вызывающем коде по признаку Result: Fail или Ok понять, штатно ли всё отработало или были проблемы.  
Теперь без проброса исключения в вызывающий код, мы можем корректно обработать неуспешное выполнение.  
Перейдем в вызывающий код - он находится в `ProductHandler`.  
В этом слое не будем ничего менять - просто пробросим Result дальше в Presentation слой.
...
- если в репозитории метод выполнился успешно - то нам нужно обработать результат, т.к. у нас теперь возвращается не коллекция Product, а коллекция завернутая в Result - обратимся к полезной нагрузке этого Result, данные в Result хранятся в свойстве Value. 
```csharp
...
[INFO Статус-коды сервера](INFO/Статус-коды%20сервера.md)  
Рассуждаем примерно так - если Result вернулся не успешный - мы должны вернуть какой-то "плохой" статус-код.
Какой из статус кодов возвращать при какой из ошибок - здесь нет универсальных рецептов, нужно анализировать кейсы и принимать решения.  
Для того чтобы удобно работать с ответами контроллера добавим использование класса `ActionResult` в сигнатуры методов  
[INFO ActionResult](INFO/ActionResult.md)  
Изменим код контроллера  
- Добавим `ActionResult` в сигнатуры методов 
- Добавим возврат 503 статуса-кода, если хэндлер вернул неуспешный Result  
- Добавим извлечение значения из Result, для удобной работы  
![](attachments/Pasted%20image%2020250207161102.png)
Запустим приложение и посмотрим что изменилось  
![](attachments/Pasted%20image%2020250207161337.png)  
Видим, что получили Code - 503, указанный нами текст ошибки, и в консоли мы бы "залогировали" исключение  
Важно обратить внимание, что всегда важно в таких ситуациях где-то фиксировать исключение - чтобы понимать, какая реально была проблема. Т.к. когда клиент АПИ придет с текстом ошибки, если он будет очень общий и использоваться в многих местах кода - может не оказаться возможности найти проблемное место, стек трейс из исключения позволяет понять место проблемы более четко.  
Для фиксации исключений нужно использовать логи и метрики, о которых мы поговорим далее по курсу. А вот отдавать исключения напрямик клиенту не очень хорошо, хотя бывают разные ситуации.
Запустим контейнеры в докере и проверим, что штатная работа также сохранена  